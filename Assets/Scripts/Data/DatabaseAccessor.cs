...
public class DatabaseAccessor
{
    ...
    public static IEnumerator FetchUserData(string titleId, TMP_Text loginStatusText)
    {
        ...
        {
            var json = (Dictionary<string, object>)PlayFabSimpleJson.DeserializeObject(request.downloadHandler.text);
    ...
    {
        using var request = UnityWebRequest.Post(RestHost + UpdateUserEndpoint, PlayFabSimpleJson.SerializeObject(userData), "application/json");
        request.SetRequestHeader("Authorization", DatabaseToken);
        yield return request.SendWebRequest();
        if (request.result != UnityWebRequest.Result.Success)
        {
