    ...
    public class World : MonoBehaviour
    {
        ...
        public static World
            Deserialize(Dictionary<string, object> data,
                Vector3 lastPosition) // Getting the player's last position allows us to only render chunks around the player.
        {
            ...
            else
            {
                ...
                var split = rawWorldPos
                    .Split(","); // We can ignore these warnings as these values are validated earlier.
        ...
        private void Awake()
        {
            if (TextureAtlas.GetAtlas() == null)
                new GameObject("TextureAtlas").AddComponent<TextureAtlas>();
        ...
        private void GenerateWorld()
        {
            ...
            var xMax = chunkPosition.x + _renderDistance;
            var zMax = chunkPosition.y + _renderDistance;
            _renderedChunks.Clear();
            var updated = new List<Chunk>();
            for (var x = xMax; x > (originX - _renderDistance); x--)
            {
                for (var z = zMax; z > (originZ - _renderDistance); z--)
                {
            ...
            foreach (var c in from c in old
                     from n in updated.Where(n => c.chunkCoord.x == n.chunkCoord.x && c.chunkCoord.y == n.chunkCoord.y)
                     select c)
            {
                c.Unload();
        ...
        public Block GetHighestAt(int x, int z)
        {
            ...
            for (var y = 0; y < Chunk.ChunkHeight; y++)
            {
                ...
                if (block != null && !block.Type.Equals(ItemType.Get(ItemType.Keys.Air)) &&
                    block.Position.y > highest.Position.y)
                    highest = block;
            }
        ...
        public MeshData BuildMeshData(Chunk chunk)
        {
                ...
                for (var d = -1; d < chunkParams[axis]; d++)
                {
                    ...
                    for (var v = 0; v < chunkParams[axisV]; v++)
                    {
                        for (var u = 0; u < chunkParams[axisU];)
                        {
                            ...
                            AddQuad(vertices, triangles, uvs, textureIndices, ref vertexIndex, quadPos, dirU, dirV,
                                width,
            ...
            return new MeshData
                { Vertices = vertices, Triangles = triangles, UVs = uvs, TextureIndices = textureIndices };
        }
